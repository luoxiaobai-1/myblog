            ...
            <div class="tmsg-commentshow">
                <ul class="tmsg-commentlist">
                    <li class="tmsg-c-item" v-for="(item,index) in commentList" :key="'common'+index">
                        <article class="">
                            <header>
                                <img  :src=" item.avater"  :onerror="$store.state.errorImg">
                                <div class="i-name">
                                    {{item.username}}
                                </div>
                                <div class="i-class">
                        ...
                        <ul v-show="item.children" class="tmsg-commentlist" style="padding-left:60px;">
                            <li class="tmsg-c-item" v-for="(citem,cindex) in item.children" :key="'citem'+cindex">
                                <article class="">
                                    <header>
                                            <img :src="citem.avater"  :onerror="$store.state.errorImg">
                                            <div class="i-name">
    ...
    export default {
        data() { //选项 / 数据
            return {
                ...
                listDom:'',//评论列表
                tmsgBox:'',//总评论盒子
                img:'https://tse1-mm.cn.bing.net/th/id/OET.1c8af9d4944648beb34508f8e77c88fe?w=135&h=272&c=7&rs=1&o=5&pid=1.9',
                // 查询参数
                queryParams: {
                    pageNum: 1,
                    pageSize: 10,
